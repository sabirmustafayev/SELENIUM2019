...
public class TestNGReview {
    ...
    //if driver closing at @aftermethod annotaion so it should be created at @beforemethod
    @AfterMethod
    public void teardown(){
    ...
    public void verifyGooglePageTitle(){
        System.out.println("Test 1");
        driver.get("http://google.com");
        String expectingTitle = "Google";
        String actualTitle = driver.getTitle();
        Assert.assertEquals(actualTitle, expectingTitle,"Title is not correct!");
    ...
    public void verifyApplesPageTitle(){
        ...
        String expectingTitle = "Apple";
        String actualTitle = driver.getTitle();
        //message will be printed only if assertion failed
        Assert.assertEquals(actualTitle, expectingTitle,"Title is not correct!");
        //if assertion failed, line below will not be reachable
        //if assertion passed,u will see message from line below
        System.out.println("Test Passed");
    }

    //data provider can supply test with a test data. also, it allows to do Data Driven Testing
    //What is it? it is when same test runs multiple times with different test data set
    public static Object[][] testData() {
        return new Object[][]{{"https://www.apple.com/", "Apple"}, //1st set fo data
                {"https://google.com/", "Google"} //2nd set of data
        };
